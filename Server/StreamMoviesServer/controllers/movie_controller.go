...
import (
	...
	"github.com/Niteesh-Kulhari/StreamMovies/Server/StreamMovieServer/utils"
...
func AddMovie() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error while inserting movie", "detail": err.Error()})
			return
		}
...
func GetRankings() ([]models.Ranking, error) {
	...
	if err := cursor.All(ctx, &ranking); err != nil {
		return nil, err
	}
...
func GetRecommendedMovies() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		recommendedMovieLimitStr := os.Getenv("RECOMMENDED_MOVIE_LIMI")
...
func GetUsersFavGenres(userId string) ([]string, error) {
	...
	if !ok {
		return []string{}, errors.New("unable to retrieve favourite genres for useer")
	}
